...
module.exports = class extends Base {
  async indexAction() {
    ...
    const goodsList = await model.where({is_delete: 0, is_on_sale: 1}).select();
  ...
  async skuAction() {
    ...
    const model = this.model('goods').where({is_delete: 0, is_on_sale: 1});
  ...
  async detailAction() {
    ...
    return this.success({
      info: info,
      gallery: [{img_url: info.primary_pic_url}],
      attribute: attribute,
  ...
  async listAction() {
    ...
    // 搜索到的商品
    const goodsData = await goodsQuery.where(whereMap).field(['id', 'name', 'primary_pic_url', 'retail_price']).order(orderMap).page(page, size).countSelect();
  ...
  async filterAction() {
    ...
    goodsQuery.where({is_delete: 0, is_on_sale: 1});
  ...
  async relatedAction() {
    ...
    if (think.isEmpty(relatedGoodsIds)) {
      ...
      relatedGoods = await model.where({category_id: goodsCategory.category_id}).field(['id', 'name', 'primary_pic_url', 'retail_price']).limit(8).select();
    } else {
      relatedGoods = await model.where({id: ['IN', relatedGoodsIds]}).field(['id', 'name', 'primary_pic_url', 'retail_price']).select();
    }
